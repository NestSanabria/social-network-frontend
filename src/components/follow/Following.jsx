...
export const Following = () => {
  ...
  const follow = async (userId) => {
    ...
    if (data.status === "success") {
      ...
      incrementFollowing(); // Incrementa el contador de seguidos
    }
  };
  ...
  const unfollow = async (userId) => {
    ...
    if (data.status === "success") {
      ...
      decrementFollowing(); // Decrementa el contador de seguidos
    }
  };
  ...
  return (
    <div>
      ...
      <div className="people__list">
        {users.length === 0 ? (
          <div>No hay usuarios seguidos para mostrar.</div>
        ) : (
          users.map((user) => (
