// src/components/user/FeedGlobal.jsx
...
export const FeedGlobal = () => {
  ...
  const fetchPublications = async (currentPage) => {
    ...
    try {
      const response = await axios.get(`${Global.url}publication/feed-global`, {
        headers: {
          Authorization: token,
        },
      ...
      if (response.data.status === 'success') {
        // Verificamos si hay nuevas publicaciones
        if (currentPage > 1) {
      ...
      console.error("Error fetching global publications:", err);
  ...
  return ( 
    <div className="user-publications-container"> {/* Contenedor principal */}
      <header className="user-publications-header">
        <h1 className="user-publications-title">Feed Global</h1>
      </header>
